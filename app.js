  ...
  function renderPaymentPending(payment) {
    ...
    main.innerHTML = `
      <section class="paywall status-card">
        ...
        <p class="disclaimer">You can keep this Mini App open. Status is checked automatically.</p>
      </section>`;
    ...
    state.paymentTimer = setInterval(checkPaymentStatus, 10000);
  }

  async function checkPaymentStatus() {
    if (state.busy || state.screen !== "payment-pending") return;
    state.busy = true;
    try {
      ...
      // Automatic polling remains silent; the manual button can be used again.
    } finally {
      state.busy = false;
    }
  }
