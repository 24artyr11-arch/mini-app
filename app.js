(() => {
  ...
  function aboutMentalTrader() {
    return `
      <section class="about-video-card" aria-label="MENTAL TRADER signal visualization">
        <video
          class="about-demo-video"
          autoplay
          muted
          loop
          playsinline
          ...
          aria-label="Animated MENTAL TRADER trading signal demonstration"
        >
          <source src="./assets/mental-trader-demo.mp4?v=6.9" type="video/mp4">
        </video>
      </section>`;
  }
  ...
  function renderPaymentPending(payment) {
    ...
    // Legacy pending payments can be loaded directly on app startup, before the
    // current checkout state exists. Preserve the old plan so the user can
    // cancel and go straight back to payment-method selection.
    if (!state.checkout.plan && payment?.plan_key) {
      state.checkout.plan = payment.plan_key;
    }
    ...
    main.innerHTML = `
      <section class="paywall status-card">
        ...
        ${automatic && state.checkout.intent?.address
          ...
          : ""}
        <button type="button" id="cancelPaymentBtn" class="cancel-payment-btn wide-btn">Cancel Payment & Change Method</button>
        <button type="button" id="pendingRefreshBtn" class="retry-btn wide-btn">Refresh Mini App</button>
        <p class="disclaimer">${automatic
          ? "Status is checked automatically every 10 seconds. You can cancel an unfinished attempt and choose another payment method."
          : "If this payment was not completed, you can cancel the attempt and choose another payment method."}</p>
      </section>`;
    ...
    document.getElementById("cancelPaymentBtn")?.addEventListener("click", cancelPaymentAndChooseMethod);
  ...
  async function cancelPaymentAndChooseMethod() {
    if (state.busy) return;

    const confirmed = window.confirm(
      "Cancel this unfinished payment and choose another payment method?\n\n" +
      "If you already sent funds, do not cancel. A crypto transfer cannot be reversed."
    );
    if (!confirmed) return;

    state.busy = true;
    clearPaymentTimer();
    const button = document.getElementById("cancelPaymentBtn");
    if (button) {
      button.disabled = true;
    ...
    try {
      const previousPlan = state.checkout.plan;
      ...
      // Users with old pending payments may arrive here immediately after
      // bootstrap, so payment options might not have been loaded yet.
      ...
      const planStillExists = state.paymentOptions?.plans?.some(
        (plan) => plan.id === state.checkout.plan
      );

      if (planStillExists) {
        renderPaymentMethods();
      } else {
        state.checkout.plan = null;
        renderPlans();
      }
    } catch (error) {
      setPaymentStatusNotice(`Could not cancel payment: ${error.message}`, true);
      if (button) {
        button.disabled = false;
        button.textContent = "Cancel Payment & Change Method";
      }
      state.paymentTimer = setInterval(() => checkPaymentStatus(false), 10000);
    } finally {
      state.busy = false;
