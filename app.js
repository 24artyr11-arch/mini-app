(() => {
      ...
      <section class="paywall status-card">
        ...
        ${automatic && state.checkout.intent?.address
          ? `<button type="button" id="backToPaymentBtn" class="retry-btn wide-btn">Back to Payment</button>
             <button type="button" id="cancelPaymentBtn" class="cancel-payment-btn wide-btn">Cancel Payment</button>`
          : ""}
        <button type="button" id="pendingRefreshBtn" class="retry-btn wide-btn">Refresh Mini App</button>
        <p class="disclaimer">${automatic
          ? "Status is checked automatically every 10 seconds. You can return to the details or cancel this attempt and choose another payment method."
  ...
  async function cancelPaymentAndChooseMethod() {
    ...
    const confirmed = window.confirm(
      "Cancel this payment attempt and choose another payment method?\n\n" +
      "If you already sent funds, do not cancel. The blockchain transfer cannot be reversed."
    );
    if (!confirmed) return;
